...
def calculate_payout():
    ...
    try:
        ...
        # Insert the total payout into the TotalSalary table
        insert_query = """
        INSERT INTO TotalSalary (employee_id, total_payout)
        VALUES (%s, %s)
        """
        cursor.execute(insert_query, (employee_id, total_payout))
        mysql.connection.commit()
 
        cursor.execute('SELECT * FROM TotalSalary')
        result1 = cursor.fetchall()
        result1_dict = convert_to_dict_list(cursor, result1)
        write_to_csv('TotalSalary', result1_dict)
        update_json('TotalSalary', result1_dict)
